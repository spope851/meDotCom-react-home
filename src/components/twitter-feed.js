...
export const TwitterFeed = () => {
    ...
    useEffect(() => {
        (async () => {
            const tweetsReq = await fetch(`api/getTweets?count=${count}`).then(res => res.json())
            setTweets(tweetsReq)
        })()
    }, [count])
            ...
            {tweets.map(tweet => {
                ...
                const extendedEntities = rt
                    ? tweet.retweeted_status.extended_entities && tweet.retweeted_status.extended_entities.media.map((entity, _idx, arr) => (
                        <img key={entity.id} src={entity.media_url_https} width={`${100 / arr.length}%`} style={{ borderRadius: 10 }} alt="tweet content"/>
                    ))
                    : tweet.extended_entities && tweet.extended_entities.media.map((entity, _idx, arr) => (
                        <img key={entity.id} src={entity.media_url_https} alt="tweet content" width={`${100 / arr.length}%`} style={{ borderRadius: 10 }} />
                    ))
                const favorites = rt
